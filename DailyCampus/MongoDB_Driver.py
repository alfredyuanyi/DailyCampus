...
class MongoDB_Driver:
    ...
    '''
    def __init__(self,db_ip='127.0.0.1',db_port=40029,database_name='南京邮电大学'):
    ...
    同db_findone
    '''
    def db_findAll(self,collection,condition):
        self.col=self.database[collection]
        cursor=self.col.find(condition)
        return cursor
    '''
    ...
    def db_remove(self,collection,dictionary):
        ...
        try:
            ...
            return True
    def db_getCollectionNames(self):
    ...
    def db_findSort(self, collection, condition,sortcondition):
        cursor = self.db_findAll(collection,condition).sort(sortcondition)
        return cursor
        pass


    def db_update_one(self, collection, condition, updata):
        self.col = self.database[collection]
        result = self.col.update_one(condition, update)
        return result
        pass
